...
Works with RPI camera module using legacy picamera or libcamera, picamera2 python3 module.
...
def speed_camera():
    ...
    display_speed = None      # last confirmed speed for live overlay
    display_speed_time = 0    # when it was recorded
    start_pos_x = None
    end_pos_x = None
    prev_pos_x = None
    ...
    while still_scanning:
                ...
                if (abs(end_pos_x - prev_pos_x) >= MO_MIN_X_DIFF_PX and
                    ...
                    if track_count >= MO_TRACK_EVENT_COUNT:
                        ...
                        if IM_FIRST_AND_LAST_ON or IM_SAVE_4AI_ON:
                            mo_im_last = image2
                        if MO_MAX_SPEED_LIMIT > 0 and ave_speed > MO_MAX_SPEED_LIMIT:
                            logging.info(
                                "End  - Skip  SPEED %.1f %s exceeds MO_MAX_SPEED_LIMIT=%i"
                                " (likely two cars) D=%i C=%i",
                                ave_speed,
                                speed_units,
                                MO_MAX_SPEED_LIMIT,
                        ...
                        elif ave_speed > MO_MAX_SPEED_OVER or CALIBRATE_ON:
                            display_speed = ave_speed
                            display_speed_time = time.time()
                            logging.info(
        ...
        if GUI_WINDOW_ON:
            if GUI_IMAGE_WIN_ON:
                image2 = speed_image_add_lines(image2_copy, cvRed)
                big_image = cv2.resize(image2, (image_width, image_height))
                if display_speed is not None and time.time() - display_speed_time < 5:
                    speed_label = "%.1f %s" % (display_speed, speed_units)
                    cv2.putText(big_image, speed_label, (10, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.0, cvWhite, 3)
                    cv2.putText(big_image, speed_label, (10, 30),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.0, cvBlack, 1)
