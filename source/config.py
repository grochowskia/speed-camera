...
CAL_OBJ_PX_L2R = 169      # L2R Moving Objects, Length of a calibration object in pixels  (2023 Genesis GV60)
CAL_OBJ_MM_L2R = 4515     # L2R Moving Objects, Length of the calibration object in millimetres
CAL_OBJ_PX_R2L = 168      # R2L Moving Objects, Length of a calibration object in pixels  (2023 Genesis GV60)
CAL_OBJ_MM_R2L = 4515     # R2L Moving Objects, Length of the calibration object in millimetres
...
LOG_FPS_ON = True         # True= Show average frame count every 1000 loops False= Off

# Camera Settings
# ---------------
CAMERA = "pilibcam"    # valid values: pilibcam (Pi camera via picamera2), usbcam, rtspcam
...
# Camera Colour Settings (pilibcam only)
# ---------------------------------------
# AWB mode:  None=use ColourGains below  0=Auto 1=Tungsten 2=Fluorescent 3=Indoor 4=Daylight 5=Cloudy
CAM_AWB_MODE = 0             # 0=Auto 1=Tungsten 2=Fluorescent 3=Indoor 4=Daylight 5=Cloudy
CAM_COLOUR_GAINS = None      # None=don't override auto gains
CAM_SATURATION = 1.0         # 1.0=normal — establish clean baseline before tweaking
...
MO_TRACK_EVENT_COUNT = 2    # Default= 6 Number of Consecutive Motion Events to trigger speed photo. Adjust to suit.
...
MO_MAX_X_DIFF_PX = 100      # Default= 20 Exclude if max px away >= last motion event x position
...
MO_EVENT_TIMEOUT_SEC = 0.5  # Default= 0.3 seconds to wait for next motion event before starting new track
MO_MAX_SPEED_OVER = 20      # Exclude track if Speed less than or equal to value specified 0=All
                            # Can be useful to exclude pedestrians and/or bikes, Etc or track only fast objects
MO_MAX_SPEED_LIMIT = 90    # Exclude track if Speed greater than this value (mph or kph) 0=off
                            # Filters ludicrous readings caused by two cars crossing simultaneously
...
MO_CROP_X_LEFT = 100          # Default=50  (doubled for 640x480)
MO_CROP_X_RIGHT = 500         # Default=250 (doubled for 640x480)
MO_CROP_Y_UPPER = 180         # Default=90  (doubled for 640x480)
MO_CROP_Y_LOWER = 300         # Default=150 (doubled for 640x480)
...
CV_WINDOW_BIGGER = 0.5        # Default= 1.0 min=0.1 Resize multiplier for opencv window if GUI_WINDOW_ON=True
